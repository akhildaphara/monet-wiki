...
### SwiftUI & Apple HIG Best Practices

When designing or implementing for iOS, macOS, or other Apple platforms using SwiftUI, adhere strictly to the Apple Human Interface Guidelines (HIG):

- **Native Primitives First**: Always prefer standard components (`NavigationStack`, `List`, `TabView`) over custom rebuilds to inherit accessibility, platform conventions, and performance.
- **Dynamic Type**: Never hardcode font sizes. Use semantic fonts (`.font(.body)`, `.font(.headline)`) to respect user accessibility settings and scale gracefully.
- **Semantic Color & Appearance**: Use standard semantic colors (e.g., `.primary`, `.secondary`, `.background`) and system materials (`.ultraThinMaterial`). Design for both Light and Dark mode seamlessly by relying on these system-adaptive colors.
- **Fluid Layouts**: Rely on `VStack`, `HStack`, `ZStack`, and adaptive padding. Avoid hardcoded `.frame(width:height:)` unless strictly required for a fixed visual element. Let content dictate the size.
- **Purposeful Motion**: Use SwiftUI's native spring animations (`.spring()`, `.bouncy()`) instead of basic linear or ease-in/out curves for a physical, responsive feel.
- **Accessibility by Default**: Ensure every custom UI element has appropriate `.accessibilityLabel`, `.accessibilityHint`, and `.accessibilityTraits`. Group complex elements with `.accessibilityElement(children: .combine)`.
- **Haptics & Feedback**: Integrate subtle haptic feedback (`sensoryFeedback`) for significant interactions, matching the visual feedback.
