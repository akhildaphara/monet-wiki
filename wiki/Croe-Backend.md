The [[Croe-Backend]] is the backend server for the [[Monet-App-Overview]] application. Built using Node.js and Express in TypeScript, it is responsible for providing all backend logic, serving APIs to the [[Monet-iOS-App]], handling user authentication via Google Auth, processing transactions with the [[Plaid-Integration]], and managing data persistence through [[Database-Schema]] in AWS DynamoDB.
...
- **External APIs**: Plaid API for transactions, Google Generative AI for certain data enrichment features.
...
- `plaid.ts`: The [[Plaid-Integration]] layer for securely fetching and formatting user financial data.
