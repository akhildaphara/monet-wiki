...
### P0 — Public App Store compliance: account deletion verified end-to-end — completed 2026-09-17

Registered users can delete their account from iOS or the website through authenticated, re-confirmed flows. The `DELETE /v1/account` implementation purges credentials, linked user profiles, transactions, overrides, preferences, feedback, insights caches, all Plaid index rows owned by those users, and form submissions matching the account email, with best-effort Plaid item revocation. On 2026-09-17, end-to-end account deletion was verified on a deployed TestFlight build: account deletion through Settings, app state return to guest onboarding, subsequent login rejected, and backend account-scoped records purged. Complies with [Apple App Review Guideline 5.1.1(v)](https://developer.apple.com/app-store/review/guidelines/). Installation, global cache/audit, and IP-rate-limit rows have no account linkage and are not deleted by this route.
...
### P0 — Deploy and validate guest abuse controls before app-key rollout — completed 2026-09-17

The guest-token route no longer requires `X-Monet-App-Key`; the website no longer sends the source-visible value; and backend/iOS CI source fallbacks have been removed. The deployed backend enforces a shared 100-mints/day quota per trusted CloudFront viewer (`CloudFront-Viewer-Address` hashed via HMAC-SHA256), 20 queries/day per guest ID plus a 200/day per-viewer ceiling, and fail-closed guest quota errors in DynamoDB. A path-scoped CloudFront WAF rule blocks bursts over 60 guest-mint requests per five minutes per IP. On 2026-09-17, live guest token issuance was verified against `api.tapmonet.com` returning HTTP 200 with a valid guest JWT.
...
### P1 — Observability and operational alerting — completed 2026-09-17

The backend includes Plaid/Google/Bedrock failure-rate alarms, core and auxiliary DynamoDB throttle alarms, CloudFront 5xx monitoring, and an operations runbook. On 2026-09-17, the deployed dev-stage SNS email subscription (`akhildaphara@gmail.com`) was confirmed active, and all 8 CloudWatch alarms were verified enabled, connected to that topic, and reporting `OK`. MetricKit telemetry is active on iOS with local persistence and feedback attachment.
